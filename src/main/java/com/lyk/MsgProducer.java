...
public class MsgProducer {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "127.0.0.1:9092");
         /*
             ...
             （2）acks=1： 至少要等待leader已经成功将数据写入本地log，但是不需要等待所有follower是否成功写入。就可以继续发送下一条消息。
                  这种情况下，如果follower没有成功备份数据，而此时leader挂掉，则消息会丢失。
        ...
        for (int i = 1; i <= msgNum; i++) {
            ...
            //指定发送分区
            ProducerRecord<String, String> producerRecord = new ProducerRecord<String, String>("test1", 0, order.getOrderId().toString(), JSON.toJSONString(order));
